...
begin
  ...
  simp [sum_left_fv] at fv_e,
  cases et_ih fv_e,
  left,
  exact val.sum_left h,
  right,
  cases h,
  existsi exp.sum_left et_τ2 h_w,
  exact steps.sum_left_arg h_h,
  simp [sum_right_fv] at fv_e,
  cases et_ih fv_e,
  left,
  exact val.sum_right h,
  right,
  cases h,
  existsi exp.sum_right et_τ1 h_w,
  exact steps.sum_right_arg h_h,
  ...
  cases sum_canonical_forms h et_a,
  cases h_1,
  rw h_1 at fv' h ⊢,
  rw sum_left_fv w at fv',
  existsi subst w et_x1 fv'.left et_e1,
  exact steps.case_done_left fv'.left h,
  rw h_1 at fv' h ⊢,
  rw sum_right_fv w at fv',
...
begin
  ...
  rw h.left,
  split,
  exact has_typ.sum_right a.left,
  simp [sum_right_fv],
  exact a.right,
  simp [case_never_fv] at fv_e,
...
begin
  ...
  refl,
  cases h2,
  rw (typ.sum.inj (h1_ih_a h2_a)).left at h1_ih_a_1,
  exact h1_ih_a_1 h2_a_1,
end
